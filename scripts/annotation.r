...
if (opt$a=='FC') {
	...
	path='output/tophat_fusion/'} else if (opt$a=='CDBG') {
    ...
    path='output/UROBORUS/'} else if (opt$a=='DCC') {
    path='output/DCC/'} else if (opt$a=='SE') {
...
ref_hsa=read.csv('ref_circ/remapped_hsa_hg19_circRNA.csv',header = T)
