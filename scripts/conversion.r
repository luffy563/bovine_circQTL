...
for (i in 1:length(filenames[,1])) {
	...
	for (j in 1:length(data$V4)) {
	  # Adjusting the V1 from '1' to 'chr1'
	  if (opt$a=='MP') {
	  ...
	  # Adjusting the V4 name of circRNAs
	  if (opt$q==TRUE) {
	    V4[j]=paste(V1[j],':',data$V2[j],'|',data$V3[j],sep = '')
	  } else {
		if (opt$a=='CDBG') {
		  V4[j]=paste('btu_circRNA_', j, '/', as.character(data$'reads'[j]),sep="") } else if (opt$a=='CM') {
		  V4[j]=paste('btu_circRNA_', j, '/', as.character(data$'reads'[j]),sep="") } else if (opt$a=='CIRI2') {
		  ...
		  V4[j]=paste(as.character(data$V1[j]),':',as.character(data$V2[j]),'-',as.character(data$V3[j]),'/',as.character(data$V5[j]),sep="") } else if (opt$a=='CIRC2') {
		  V4[j]=paste(as.character(data$V4[j]),sep="") } else {
